...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:UML="omg.org/UML1.3" exclude-result-prefixes="UML"> 
	...
	<xsl:template match="/XMI">
		<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
			...
			<xsl:call-template name="namespace">
				...
				<xsl:with-param name="uri" select="substring-before(./XMI.content/UML:TaggedValue[@tag='targetNamespace']/@value,'#')"/>
			</xsl:call-template>

			<!--
				Modify the following in accordance to the namespace(s) involved
			-->
			...
			<!--
			<xsl:call-template name="namespace">
				<xsl:with-param name="prefix" select="'iwxxm'"/>
				<xsl:with-param name="uri" select="'http://icao.int/iwxxm/3.0'"/>
			</xsl:call-template>
			-->
			<xsl:call-template name="namespace">
			...
			<xsl:element name="sch:pattern">
				<xsl:attribute name="id">
					<xsl:value-of select="'COLLECT.MB1'"/>
				</xsl:attribute>
				<xsl:element name="sch:rule">
					<xsl:attribute name="context">
						<xsl:value-of select="'//collect:MeteorologicalBulletin'"/>
					</xsl:attribute>
	...
	<xsl:template name="pattern-codeList">
		...
		<xsl:element name='sch:pattern'>
			...
			<xsl:element name='sch:rule'>
				<xsl:attribute name="context">
					<xsl:value-of select="concat('//*[contains(name(),''',$parentName,''')]/',$prefix,':',$selfName)"/>
				</xsl:attribute>
				<xsl:element name='sch:assert'>
					<xsl:attribute name="test">
