...
<!--
	...
	Created by B.L. Choy (blchoy.hko@gmail.com).  First created on 29 March 2016.  Last updated on 19 July 2021.
...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2001/XMLSchema" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes=" xs"> 
	...
	<xsl:template xmlns:xs="http://www.w3.org/2001/XMLSchema" match="xs:schema">
		<xsl:element name='{local-name()}'>
			<xsl:if xmlns:xs="http://www.w3.org/2001/XMLSchema" test="./xs:element[@name='Report']">
				<xsl:attribute name="xmlns:aixm" select="'http://www.aixm.aero/schema/5.1.1'"/>
			</xsl:if>
			<xsl:apply-templates select='@*|node()' />
		</xsl:element>
	</xsl:template>
	...
	<!-- New directory structure for the schemas starting from Version 2021-2 -->
	<xsl:template xmlns:xs="http://www.w3.org/2001/XMLSchema" match="xs:include[@schemaLocation!='']">
		<xsl:choose>
			<xsl:when xmlns:gml="http://www.opengis.net/gml/3.2" test="../xs:annotation/xs:appinfo/gml:gmlProfileSchema = 'gmliwxxm.xsd'">
				<xsl:element name='{local-name()}' >
					<xsl:attribute name="schemaLocation">
